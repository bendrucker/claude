...
- `import_hash` fingerprints `00_pinned.sql`. On a mismatch `reconcilePinned` runs `CREATE OR REPLACE TABLE raw AS SELECT host, UNNEST(pinned_columns(data)), source_file, source_line, data FROM raw`, which adds columns the projection gained, drops ones it lost, and recomputes the rest in place. A null hash is an index that predates the check, whose rows already came from this projection, so the fingerprint is recorded without a rewrite. Adopting one still rebuilds what is derived from `raw`, because the code that built it could leave a `content_items` behind a committed import and the index reads as current either way. The rewrite materializes a second copy of the table, and DuckDB never returns freed space, so it checkpoints and leaves `refresh.ts`'s compaction guard to shrink the file.
