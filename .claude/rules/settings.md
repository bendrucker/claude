...
User-level settings live in `user/settings.json` (plugins, permissions, sandbox). Project-level settings live in `.claude/settings.json` (ox hook). See the [settings documentation](https://code.claude.com/docs/en/settings) for available options.
