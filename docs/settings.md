...
`CLAUDE_CODE_SUBAGENT_MODEL=opus` is the model for a spawn that names none at the call site or in the agent's frontmatter. Explicit params, pinned agents, and `model: inherit` (the built-in `Plan`, [`user/agents/review.md`](../user/agents/review.md)) all rank above it. The target is a Fable parent, where a bare `general-purpose` spawn otherwise inherits Fable: 150 did over July and August 2026, and the 35 implementation agents among them would have done the same work on Opus at half the rate. Under a Sonnet or Haiku parent it upgrades a bare spawn to Opus, which the `agent-model` hook flags. `Explore` already carries this cap in the CLI binary.
