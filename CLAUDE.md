...
`pull-request:create`, `pull-request:follow-up`, and `review:follow-up` each carry a `promptfooconfig.yaml` under their `evals/` dir: an in-repo promptfoo suite that loads the plugin and grades cases with `llm-rubric` asserts. Those three run manually. A native `claude plugin eval` suite is a directory of `<case>/case.yaml` cases plus a `run.sh` that loads its plugins and runs on subscription auth, as in [`plugins/pull-request/evals/create`](plugins/pull-request/evals/create/) and [`plugins/writing/evals/no-diary`](plugins/writing/evals/no-diary/). [`evals/scripts/`](evals/scripts/) files promptfoo runs into the durable corpus and reports what they cost.
