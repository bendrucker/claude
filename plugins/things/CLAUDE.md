...
`src/mcp/tools.ts` maps a write tool's arguments through one `ATTRIBUTES` table rather than per handler. `registerTools` takes a `ThingsClient`, so a test drives a tool call through an in-memory MCP round trip against a fake. Both are documented in [`src/mcp/README.md`](src/mcp/README.md).
