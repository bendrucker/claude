...
4. Fan out one agent per dimension below, each with `subagent_type: analyst`. Every dimension reads the index and reports, which is what that agent is for, and it carries its own Sonnet default so the fan-out does not bill at an Opus orchestrator's rate. Each runs its named queries (`duckdb -readonly -json <db-path> < resources/queries/<name>.sql`, scoped with `SET VARIABLE`) plus any inline rollups, all read-only, all against the one shared file. Read-only opens share the lock, so the agents never contend with each other.
...
- Split friction into the part a setting can change and the part it cannot. `ExitPlanMode` and `AskUserQuestion` rejections are not allowlistable; the named `permissions` and `errors` queries already exclude them, so only custom SQL over `permission_requests`/`tool_errors` needs that filter. A compound command still matches `excludedCommands` at any of its top-level positions, so a bypass on `cd <dir> && <exempt verb>` had no sandbox cause and no setting will retire it. Counting raw rejections without that split inflates the actionable set.
- A query studying the plan gate specifically cannot trust `denial_kind` on `ExitPlanMode`. The harness sets `$.toolDenialKind` to `permission-rule` for any PreToolUse-shaped deny, which covers plannotator's structured feedback block and plain freeform human rejections alongside an actual `plugins/plan/hooks/gate.ts` deny. Identify a genuine gate deny by matching `error_content` against the four rules the gate enforces, using substrings that survive its rewordings: `unchanged` or `byte-identical` for unchanged text, `nearly every` or `nearly all` for append-only, `is larger than any before it` for sustained growth, and `k characters` for size, which covers both the first-pass and the re-armed wording. The gate's phrasing has been rewritten repeatedly, so matching its current templates literally misses every earlier wording.
