...
1. Rewrite the PR body following the same title and body rules as the create skill. If a PR template is provided in context, preserve its structure. Describe _what_ is tested, not _how many_ — never mention test counts or summarize by counting items. See [`sections.md`](sections.md) for section guidance.
