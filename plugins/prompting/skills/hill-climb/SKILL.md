...
- **Lint.** Run `shellcheck` on the scaffold scripts, and test each regex grader against hand-written replies with `bun evals/native/check.ts <suite>`: one reply that should pass every grader, and one per grader that should fail it. A fix after the baseline edits a case mid-climb.
