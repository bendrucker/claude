...
} from "../../plugins/writing/linguistics/heading";
import { CODE_SENTINEL } from "../../plugins/writing/linguistics/tags";
...
export function classifyPrHeading(heading: string): PrHeadingResult {
  ...
  if (/[.](?<!\.\.\.)\s*$/.test(raw) && !/\b[A-Z]\.$/.test(raw))
    signals.push("trailing period");

  // --- Commas (sentence punctuation) --------------------------------------
  // A comma outside code/quotes signals list/clause structure. The contrast
  // idiom ", not Y" is a tight label device the user keeps ("`-F`, not `-f`"),
  // so it is exempt.
  ...
  // A short label parenthetical is fine ("(Historical)", "(Working Notes)").
  // Flag parentheticals that contain a comma or a predicate verb (a clause).
  ...
  // "Why / What / How ..." headings are rationale-section labels in this
  // corpus and the user marks the overwhelming majority bad. Flag them, but
  // require a real verb/object tail so a bare "How It Works" idiom still
  // gets caught (it is also bad here) while not over-firing on noun labels.
  ...
  // --- Subject pronoun (sentence subject) ---------------------------------
  if (lower.some((w) => SUBJECT_PRONOUNS.has(w) && w !== "that" && w !== "what")) {
    signals.push("sentence subject pronoun");
  }

  // --- Sentence case: lowercase content words -----------------------------
  // A good heading Title-Cases content words. A lowercase content word past
  // the first position means the heading is sentence case. One such word is
  // enough ("Headless fallback", "Note for the reviewer"); the first word is
  // weighted separately because some good labels open lowercase by accident.
  const lowercaseContent = toks
    .slice(1)
    .filter((t) => isContentWord(t) && /^[a-z]/.test(t));
  const firstLower =
    toks[0] && isContentWord(toks[0]) && /^[a-z]/.test(toks[0]) ? [toks[0]] : [];
  ...
  // --- Length -------------------------------------------------------------
  // Very long headings read as sentences. Count prose tokens (exclude code
  // sentinel) so a code-heavy label is not penalized.
