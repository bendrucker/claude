...
- A query studying the plan gate specifically cannot trust `denial_kind` on `ExitPlanMode`. The harness sets `$.toolDenialKind` to `permission-rule` for any PreToolUse-shaped deny, which covers plannotator's structured feedback block and plain freeform human rejections alongside an actual `plugins/plan/hooks/gate.ts` deny. Identify a genuine gate deny by matching `error_content` against the gate's message templates, pulled from `git log -p -- plugins/plan/hooks/gate.ts` rather than the current file alone: the wording has changed several times, and matching only current wording undercounts gate denials by roughly half (20 of 57 rows matched, against 43 once historical wording is included).
