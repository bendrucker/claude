...
- `~/.greptile` holds the CLI's `auth.json` beside its `reviews.json` log, and `greptile review` writes both on every run. A file-level grant does not work: the CLI chmods the directory itself, so the run dies at `could not write credentials: EPERM ... chmod '/Users/ben/.greptile'` before reaching either file. Granted so `greptile review` runs sandboxed rather than escaped, which is the same trade already accepted for the `*.greptile.com` egress entry above. The added risk is narrow, since a sandboxed process could tamper with the token but could already exfiltrate it.
