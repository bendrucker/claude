...
bun <skill-dir>/scripts/import.ts --host <label> --source '<user@host>:.claude/projects/'
```
...
bun <skill-dir>/scripts/import.ts --host <label>
```
...
```bash
bun <skill-dir>/scripts/forget.ts --host <label>
```
