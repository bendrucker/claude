...
The ranking pools every selected kind, so the null splits that same pooled population against itself and takes the largest z it reaches with no difference to find. A maximum over per-kind maxima bounds no single population: it tracks whichever kind's draw spiked, which on this corpus runs a full point above the pooled null at 5-grams and a third of a point below it at 3-grams. A 3-gram and a 5-gram over a 17-tag alphabet draw on different amounts of corpus mass, so each n-gram size carries its own floor. A size the split produced no shapes of leaves those shapes ungated and prints `n/a`, so an unmeasured size reads as unmeasured rather than as passing.
