...
`pull-request:create`, `pull-request:follow-up`, `review:follow-up`, and `writing:no-diary` each carry a `promptfooconfig.yaml` under their `evals/` dir: an in-repo promptfoo suite that loads the plugin and grades cases with `llm-rubric` asserts. Those four run manually. A native `claude plugin eval` suite is a directory of `<case>/case.yaml` cases run by [`evals/native/run.ts`](evals/native/run.ts) on subscription auth and compared with [`evals/native/compare.ts`](evals/native/compare.ts), as in [`plugins/pull-request/evals/create`](plugins/pull-request/evals/create/). A suite testing skills, agents, or context files outside a plugin wraps them through `suite.yaml`. [`evals/scripts/`](evals/scripts/) files promptfoo runs into the durable corpus and reports what they cost.
