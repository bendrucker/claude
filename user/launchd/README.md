...
`wt step prune` only removes worktrees that are merged/content-integrated into the default branch, skips dirty worktrees (uncommitted changes) unless `--force` is passed (never passed here), skips locked worktrees and the main worktree, and skips anything younger than `--min-age` (1 day, guarding freshly created worktrees that happen to sit on the same commit as main). It does not check for a live process or shell still `cd`'d into a worktree that is otherwise clean and merged — `wt remove --reap` can kill processes in a worktree being removed, but that's opt-in on `wt remove`, not automatic on `wt step prune`, and it only handles processes, not the general case of an idle interactive session sitting in a now-removed directory. 03:15 is chosen to minimize the odds of that overlap; the underlying gap is worth raising with Worktrunk as a feature request (a refusal, not just `--reap`, when a live process's cwd is under the candidate worktree).
