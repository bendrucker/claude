...
- **`content_items`**: a table declared in `01_tables.sql` and populated by `import.sql` from `unnest(json_extract(data, '$.message.content[*]'))`, one file at a time alongside `raw`. Pinned columns plus the content item's full `data JSON` plus the parent's `tool_use_result JSON`.
...
- `db.ts`: orchestrates migration, schema, scan, per-file import, view versioning, checkpoint, compaction.
