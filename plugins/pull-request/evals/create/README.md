...
`gh workflow run eval.yml --ref <branch> -f suite=plugins/pull-request/evals/create` runs it in CI. `-f args='--runs 1 --case plan-deviations'` narrows the run, and `-f baseline=<run id>` appends the `compare.ts` tables against an earlier run to the job summary.
