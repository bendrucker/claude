...
Auto-merge is passive: it arms the platform and returns. Branch protection still gates the actual merge on checks and required approvals. It does not fix red CI or wait out a review, which is what `--watch` is for.
...
- **GitHub, stacked**: auto-merge has no equivalent on a stacked PR. Say so and suggest `--watch`, which drives the stack merge at green.
