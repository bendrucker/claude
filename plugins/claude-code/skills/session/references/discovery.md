...
5. One or more grounding agents re-check every candidate against the live config (see [Grounding](#grounding)). Drop or downgrade anything that does not hold. Grounding needs `WebFetch` to check harness claims against the primary docs, which `analyst` has no access to, so leave these agents generic and pass them a cheap `model`.
