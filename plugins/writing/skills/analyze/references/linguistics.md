...
- **Cross-sentence** (negation flips, question cadence, consequence chains, tricolon): batch-surface detectors in `detection/tropes.ts`, plus the tagger-backed tricolon in `linguistics/tricolon.ts` behind the hook wall. Thresholds are literature heuristics until the #769 labeling pass calibrates them. Discourse-marker density, the subordinate:coordinate ratio and negative-contrast rate ship as rate features in `voice-delta.ts`. The permutation null rejected burstiness and a lexical shared-opener rate, leaving parallelism with `tricolon.ts`. `references/methodology.md` carries the numbers and the dispositions.
