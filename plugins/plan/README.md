...
The `PreToolUse` gate hook keeps per-session state under `/tmp/claude/<session_id>/` and runs four checks in order. It denies a resubmission whose text is unchanged from the last presentation. It denies a resubmission that keeps nearly every prior line and introduces at least one new one, which catches a line swap that nets zero growth as well as a plain append. It denies, once per session, a resubmission longer than every presentation before it, the shape of a plan that accumulates instead of shedding superseded text, measured against that high-water mark rather than the presentation just before. It denies a plan over 10k characters, the same limit the injected guidelines state, re-arming while a rework remains over the threshold, capped at two fires per session so a deny loop is impossible.
