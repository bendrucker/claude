...
const labels: Label[] = JSON.parse(
  readFileSync(join(import.meta.dirname, "labels.json"), "utf8"),
);
...
console.log(
  `FALSE NEGATIVES (bad items missed, no lexical tell) — ${falseNegatives.length}:`,
);
for (const fnItem of falseNegatives) {
  console.log(`  "${fnItem.text}"`);
  console.log(`      signals fired: ${fnItem.signals.length ? fnItem.signals.join(", ") : "(none)"}`);
}
