...
| A review bot configured for the repo (`.greptile/config.json`, `.coderabbit.yaml`) with its CLI installed | `pull-request:follow-up --local` | Reviews committed work, commits its fixes. Runs before the fix passes dirty the tree |
| Prose (`.md`, `.mdx`, `.rst`, docs) | `writing:review` | |
| A runtime surface | `verify` | Declines tests-only and docs-only itself |

Gating is the cost lever: never run a reviewer the change does not warrant. `--skip <pass>` drops any of them (`plan`, `code-review`, `simplify`, `comments`, `bot`, `writing`, `verify`).
...
flowchart TD
    ...
    G -->|no| F1[fix passes: comments-audit, local bot, code-review or simplify, writing, verify]
    F1 --> C([create PR])
    G -->|yes| D[dispatch plan:review in background]
    D --> F2[fix passes: comments-audit, local bot, code-review or simplify, writing, verify]
