...
`CLAUDE_CODE_SUBAGENT_MODEL=opus` is the model for a subagent spawn that names none, meaning no `model` at the call site and no `model:` in the agent's frontmatter. It ranks below both, so an explicit `haiku`, `sonnet`, or `fable` param and every pinned agent keep their choice. `model: inherit`, hard-coded in the built-in `Plan` and declared in [`user/agents/review.md`](../user/agents/review.md), still follows the session model. Under an Opus parent it changes nothing. Under a Sonnet or Haiku parent it upgrades a bare spawn to Opus, which the `agent-model` hook flags since it reads this default, and a spawn there can still pass `sonnet` or `haiku`. The target is a Fable parent, where a bare `general-purpose` spawn otherwise inherits Fable. Over July and August 2026, 150 spawns inherited Fable, and the 35 implementation agents among them would have done the same work on Opus at half the rate. The built-in `Explore` already carries this cap in the CLI binary. The `agent-model` hook below keeps steering bare spawns toward a cheap explicit pick, and this entry bounds what one costs when that steering is ignored.

**Drop it** if the steering alone is enough. Around 2026-10-15, run the `claude-code:session` skill's [`delegation`](../plugins/claude-code/skills/session/resources/queries/delegation.sql) query with `host` set to `local` and `after_date` to the day this shipped, and read the `generic` path under a `fable` parent. If every spawn there names a `model` or pins a type, the default never fired and goes. Also drop it when a later CLI caps `general-purpose` under Fable on its own, which the docs already claim for `Plan` and the binary does not yet do.
...
[`agent-model`](../user/hooks/agent-model) warns, on a `PreToolUse` matching `Agent`, when a spawn names neither a `model` nor a `subagent_type` that pins one and the model it resolves to is opus or fable. That model is `CLAUDE_CODE_SUBAGENT_MODEL` when the settings `env` sets one, and the parent's model otherwise. It enforces the delegation rule already written in `user/CLAUDE.md`, which spawns were ignoring: over the 30 days to 2026-09-01, 34 `general-purpose` and 30 bare spawns under opus-family parents carried no `model` and resolved to opus, and their task descriptions were lookup and fan-out shaped. It emits `additionalContext` and no `permissionDecision`, so the spawn still proceeds. `PreToolUse` carries no model field, so the parent's family comes from the last assistant record in the tail of `transcript_path`, and an unresolvable model stays silent.
