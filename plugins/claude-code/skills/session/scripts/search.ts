...
function tokenize(text: string): string[] {
  return text.toLowerCase().split(/\W+/).filter(t => t.length >= MIN_TOKEN_LENGTH)
}
...
export function calculateRelevanceScore(query: string, conversation: Conversation): { score: number; matchedContent: string[] } {
  ...
  for (const msg of conversation.messages) {
    ...
    const weight = msg.role === 'user'
      ? RELEVANCE_WEIGHTS.userMessage
      : RELEVANCE_WEIGHTS.assistantMessage
...
function loadConversations(options: SearchOptions): Conversation[] {
  const projectsDir = options.projectsDir || process.env.CLAUDE_PROJECTS_DIR || DEFAULT_PROJECTS_DIR
...
export async function searchConversations(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
