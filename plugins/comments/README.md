...
- **`apply/`**: the deterministic edit engine, which refuses to delete a required doc comment, to drop a Go doc comment's `Name` lead, or to splice a line wider than the comment it replaces, the verdict id-match against re-extracted comments, the branch writer, and the report renderer.
